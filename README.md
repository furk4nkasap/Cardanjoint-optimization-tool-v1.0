...
For double and triple Cardan configurations, the application searches
for the phase or clocking angles that produce the most effective
kinematic compensation between the velocity fluctuations generated by
consecutive Cardan joints.
...
The phase angle determines the relative angular position of the
velocity fluctuations generated by consecutive Cardan joints.

Changing the phase angle does not change the misalignment angles or
dissipate mechanical energy. Instead, it shifts the periodic velocity
fluctuation of the following joint relative to the fluctuation produced
by the preceding joint.

With a suitable phase configuration, one joint may produce a
decelerating contribution at an angular position where another joint
produces an accelerating contribution. This interaction creates
kinematic compensation and reduces the residual total velocity
unevenness.
...
that provides the greatest kinematic compensation between the periodic
velocity fluctuations of the consecutive Cardan joints.

The objective is not to introduce damping, but to minimize the residual
angular velocity unevenness at the final output shaft over one complete
input-shaft revolution.
In this formulation, the phase angles are the optimization variables,
the shaft misalignment angles remain fixed, and the residual velocity
unevenness is the objective function.

Therefore, the optimization can be interpreted as a kinematic
compensation problem:

```math
(\phi_1,\phi_2)
\longrightarrow
\text{relative fluctuation phase}
\longrightarrow
\text{kinematic compensation}
\longrightarrow
...
The objective is not to introduce physical damping, but to minimize the
residual angular velocity unevenness at the final output shaft over one
complete input-shaft revolution.

In this formulation:

- The phase angles are the optimization variables.
- The shaft misalignment angles remain fixed during each optimization run.
- The residual output-speed unevenness is the objective function.

The optimization process can therefore be summarized as:

```text
Phase angles
→ Relative fluctuation phase
→ Kinematic phase compensation
→ Minimum residual output-speed unevenness
```
...
The effect of the phase angles on the calculated response can be
summarized as:

```text
Phase angles
→ Input-angle references of the following joints
→ Individual joint velocity-ratio curves
→ Total output velocity ratio
→ Residual velocity unevenness
```

A lower optimized unevenness indicates that the individual Cardan-joint
velocity fluctuations compensate for one another more effectively.

The current software evaluates discrete phase-angle candidates and
selects the candidate, or candidate combination, that produces the
lowest calculated residual unevenness.
...
- Does not model physical damping or mechanical energy dissipation
- Represents output-speed fluctuation reduction as ideal kinematic phase compensation
