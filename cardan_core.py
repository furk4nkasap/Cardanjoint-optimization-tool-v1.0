...
# Interface contract used by streamlit_app.py. Increment this value whenever
# a future interface release requires new public objects from this module.
CORE_API_VERSION = 2
...
class PlotLabels:
    ...
    The numerical model remains language-independent. Passing translated labels
    lets the Streamlit interface localize every visible chart annotation without
    duplicating or modifying the kinematic calculations.
    """
...
def phase_combination_count(parameters: CardanParameters) -> int:
    """Return the number of discrete phase candidates evaluated by the optimizer."""
...
def plot_velocity_ratio(
    ...
    """Plot the current and optimized velocity ratios with localized labels."""
    ...
    ax.plot(
        ...
        q_current_plot,
        lw=2.6,
        label=plot_labels.current_curve,
    )
    ax.plot(
        theta_plot_deg,
        q_optimized_plot,
    ...
    annotation = (
        ...
        f"{current_unevenness:.2f}% → {status}\n"
        f"{plot_labels.optimized_unevenness} Δq/q̄ = "
        f"{result.unevenness_percent:.2f}%\n"
    )

    if parameters.mode is CardanMode.DOUBLE:
        annotation += f"{plot_labels.optimized_phi1} = {result.phi1_deg:.0f}°"
    elif parameters.mode is CardanMode.TRIPLE:
        annotation += (
            f"{plot_labels.optimized_phi1} = {result.phi1_deg:.0f}°, "
            f"{plot_labels.optimized_phi2} = {result.phi2_deg:.0f}°"
        )
...
def draw_end_view(
    ...
    direction = (
        plot_labels.direction_ccw
        if signed_phi >= 0.0
        else plot_labels.direction_cw
    )
