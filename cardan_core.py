...
# API v4 adds global/continuous optimization, validation diagnostics, and a
# renderer-independent kinematic trajectory data model.
CORE_API_VERSION = 4
...
class CardanParameters:
    ...
    def __post_init__(self) -> None:
        ...
        if not 0.0 < float(self.optimization_step_deg) <= 360.0:
...
class OptimizationSettings:
    ...
    random_seed: int = 42
    validation_sample_count: int = 7200
    polish: bool = False
    ...
    def __post_init__(self) -> None:
        ...
        if int(self.validation_sample_count) < 720:
            raise ValueError("Validation sample count must be at least 720.")
...
def _build_continuous_objective(
    ...
    if parameters.mode is CardanMode.DOUBLE:
        def objective(candidate: ArrayLike) -> float:
            phi1_rad = np.deg2rad(float(np.asarray(candidate, dtype=float)[0]) % 360.0)
    ...
    def objective(candidate: ArrayLike) -> float:
        phases = np.mod(np.asarray(candidate, dtype=float), 360.0)
...
def _result_from_phase_vector(
    ...
    phases = np.mod(np.asarray(phase_vector_deg, dtype=float), 360.0)
...
def optimize_continuous_phase(
    ...
    bounds = [(0.0, 360.0)] * (1 if parameters.mode is CardanMode.DOUBLE else 2)
    ...
    best_vector = np.mod(np.asarray(de_result.x, dtype=float), 360.0)
    ...
    if settings.method is OptimizationMethod.HYBRID:
        ...
        powell_vector = np.mod(np.asarray(powell_result.x, dtype=float), 360.0)
...
def plot_phase_figure(
    ...
    fig.suptitle(plot_labels.phase_title, fontsize=18, fontweight="bold", y=0.98)
