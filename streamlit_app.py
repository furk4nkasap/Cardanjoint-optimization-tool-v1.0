"""Bilingual Streamlit interface for Cardan Joint Engineering Tool v2.1."""
...
st.set_page_config(
    page_title="Cardan Joint Engineering Tool v2.1",
    page_icon="⚙️",
    layout="wide",
...
TRANSLATIONS: dict[str, dict[str, str]] = {
    "en": {
        ...
        "phase_help": "Positive phase follows θ_next = θ_out − φ. 360° is equivalent to 0°.",
        ...
        "objective_value": "720-point objective",
        ...
        "phase_map_note": "The heat map is the deterministic coarse grid. The star is the selected local/global optimum and can lie between grid nodes.",
...
The optimizer objective is evaluated at 720 input-shaft angles. The selected solution is then checked independently on a denser full-cycle grid. This distinguishes the optimization objective from the reported validated result.
...
""",
    ...
    "tr": {
        ...
        "phase_help": "Pozitif faz θ_next = θ_out − φ konvansiyonunu izler. 360°, 0° ile eşdeğerdir.",
        ...
        "objective_value": "720 noktalı amaç",
        ...
        "phase_map_note": "Isı haritası deterministik kaba griddir. Yıldız, seçilen yerel/global optimumu gösterir ve grid noktalarının arasında bulunabilir.",
...
Optimize edici amaç fonksiyonu 720 giriş mili açısında değerlendirilir. Seçilen çözüm daha sonra daha yoğun ve bağımsız bir tam çevrim ağı üzerinde doğrulanır. Böylece optimizasyon amaç değeri ile raporlanan doğrulanmış sonuç birbirinden ayrılır.
...
def _reset_inputs() -> None:
    ...
    st.session_state.validation_sample_count = 7200
...
with st.sidebar:
    ...
    if mode >= CardanMode.DOUBLE:
        ...
        validation_sample_count = int(st.select_slider(
            text["validation_samples"], options=(720, 1440, 3600, 7200, 14400, 36000),
            value=7200, key="validation_sample_count",
