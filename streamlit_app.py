"""Beginner-friendly Streamlit interface for Cardan Joint Engineering Tool v1.2.4."""
...
try:
    ...
    # Keep the main engineering application usable when the optional 3D
    # viewer module was not uploaded to the deployment repository.
    viewer3d = None
...
_REQUIRED_CORE_OBJECTS = (
    ...
    "ANGLE_UNIT",
    "BETA_INFORMATION_THRESHOLD_DEG",
    "BETA_WARNING_THRESHOLD_DEG",
    "UI_BETA_MAX_DEG",
    "calculate_analysis",
    "calculate_planar_scene_geometry",
    "calculate_kinematic_scene",
    "physical_phase_to_model_phase_deg",
    "model_phase_to_physical_phase_deg",
    "parameters_to_dict",
    "parameters_to_payload",
    "PhaseConvention",
...
APP_RELEASE = "1.2.4"
PHYSICAL_PHASE_CONVENTION_VERSION = 1


st.set_page_config(
    page_title="Cardan Joint Engineering Tool v1.2.4",
    page_icon="⚙️",
    layout="wide",
...
TRANSLATIONS: dict[str, dict[str, str]] = {
    "en": {
        ...
        "phase_angles": "Physical Yoke Phase Angles",
        ...
        "beta_info": "One or more active beta angles exceed 30°. Speed fluctuation and local nonlinearity may increase; verify the real joint operating range.",
        "beta_warning": "One or more active beta angles exceed 45°. Treat this as a high-angle kinematic case and verify manufacturer limits, joint life, load, and packaging separately.",
        "phase_help": "Physical yoke clocking on the shared shaft: 0° means aligned yokes and 90° means perpendicular yokes. Values separated by 180° are equivalent.",
        "phase_reference_note": "Physical reference: 0° = aligned yokes · 90° = perpendicular yokes",
        ...
        "phase_map_note": "The axes use physical yoke clocking: 0° is aligned and 90° is perpendicular. The map covers the unique 0°–180° interval; values shifted by 180° are equivalent.",
...
""",
    ...
    "tr": {
        ...
        "phase_angles": "Fiziksel Çatal Faz Açıları",
        ...
        "beta_info": "Aktif beta açılarından en az biri 30° değerini aşıyor. Hız dalgalanması ve yerel doğrusal olmayan davranış artabilir; gerçek mafsal çalışma aralığını ayrıca doğrulayın.",
        "beta_warning": "Aktif beta açılarından en az biri 45° değerini aşıyor. Bu durumu yüksek açılı kinematik vaka olarak değerlendirin; üretici limitlerini, mafsal ömrünü, yükü ve paketlemeyi ayrıca doğrulayın.",
        "phase_help": "Ortak mil üzerindeki fiziksel çatal fazıdır: 0° çatalların hizalı, 90° çatalların dik olduğu konumu gösterir. 180° farkla ayrılan değerler eşdeğerdir.",
        "phase_reference_note": "Fiziksel referans: 0° = hizalı çatallar · 90° = dik çatallar",
        ...
        "phase_map_note": "Eksenler fiziksel çatal fazını gösterir: 0° hizalı, 90° dik çatallardır. Harita benzersiz 0°–180° aralığını kapsar; 180° kaydırılmış değerler eşdeğerdir.",
...
TRANSLATIONS["en"].update({
    ...
    "viewer_module_missing": "The 3D viewer module is missing. Upload cardan_3d_viewer.py to the same GitHub folder as streamlit_app.py, then reboot the app.",
    ...
    "viewer_planar_title": "Planar 3D schematic",
    "viewer_planar_info": "This view places consecutive shafts in one common plane. Entered beta angles and physical yoke phases are preserved, but horizontal and vertical directions from a real vehicle installation are not reconstructed.",
...
TRANSLATIONS["tr"].update({
    ...
    "viewer_module_missing": "3B görüntüleyici modülü eksik. cardan_3d_viewer.py dosyasını GitHub'da streamlit_app.py ile aynı klasöre yükleyip uygulamayı yeniden başlatın.",
    ...
    "viewer_planar_title": "Düzlemsel 3B şema",
    "viewer_planar_info": "Bu görünüm ardışık milleri ortak bir düzlemde yerleştirir. Girilen beta açıları ve fiziksel çatal fazları korunur; ancak gerçek araç yerleşimindeki yatay ve düşey yönler yeniden oluşturulmaz.",
...
class _FallbackViewerLabels:
    title: str
    current: str
    optimized: str
    selected_current: str
    selected_optimized: str
    input_angle: str
    speed_ratio: str
    play: str
    pause: str
...
_ViewerLabels = viewer3d.ViewerLabels if viewer3d is not None else _FallbackViewerLabels


VIEWER_LABELS: dict[str, Any] = {
    "en": _ViewerLabels(
...
def _normalize_angle_value(
    value: float, *, min_value: float, max_value: float, step: float
) -> float:
    """Snap a widget value to the active step and clamp it to its range."""

    numeric = float(value)
    if not np.isfinite(numeric):
        numeric = float(min_value)
    clipped = min(max(numeric, float(min_value)), float(max_value))
    snapped = float(min_value) + round((clipped - float(min_value)) / float(step)) * float(step)
    return round(snapped, _step_decimal_places(float(step)))


def _sync_angle_widget(
    source_key: str, target_key: str, min_value: float, max_value: float, step: float
) -> None:
    normalized = _normalize_angle_value(
        st.session_state[source_key],
        min_value=min_value,
        max_value=max_value,
        step=step,
    )
    st.session_state[source_key] = normalized
...
def sidebar_angle_input(
    ...
    *,
    ...
    canonical_value = st.session_state.get(
        number_key, st.session_state.get(slider_key, float(default_value))
    )
    canonical_value = _normalize_angle_value(
        canonical_value, min_value=min_value, max_value=max_value, step=step
    )
    st.session_state[slider_key] = canonical_value
    st.session_state[number_key] = canonical_value
    ...
    with slider_column:
        st.slider(
            ...
            step=float(step),
            key=slider_key,
            on_change=_sync_angle_widget,
            args=(slider_key, number_key, min_value, max_value, step),
            help=help_text,
    ...
    with number_column:
        st.number_input(
            ...
            key=number_key,
            on_change=_sync_angle_widget,
            args=(number_key, slider_key, min_value, max_value, step),
            format="%.2f" if step <= 0.01 else "%.1f",
            label_visibility="collapsed",
        )
...
def _on_precision_change() -> None:
    if bool(st.session_state.get("precise_angles", False)):
        return
    maximum_by_key = {
        "beta1_deg": core.UI_BETA_MAX_DEG,
        "beta2_deg": core.UI_BETA_MAX_DEG,
        "beta3_deg": core.UI_BETA_MAX_DEG,
        "phi1_deg": core.KINEMATIC_PERIOD_DEG,
        "phi2_deg": core.KINEMATIC_PERIOD_DEG,
        "theta0_deg": core.KINEMATIC_PERIOD_DEG,
    }
    for key, maximum in maximum_by_key.items():
        number_key = f"{key}_number"
        if number_key in st.session_state:
            normalized = _normalize_angle_value(
                st.session_state[number_key], min_value=0.0, max_value=maximum, step=0.5
            )
            _set_angle_state(key, normalized)
...
def _trajectory_json(analysis: dict[str, Any]) -> bytes:
    payload = {
        ...
        "version": APP_RELEASE,
        "angles_unit": core.ANGLE_UNIT,
        "phase_convention": core.PhaseConvention.PHYSICAL_YOKE_CLOCKING.value,
        "display_revolution_deg": 360.0,
        "kinematic_period_deg": 180.0,
        "phase_definition": {
            "public": "physical_yoke_clocking",
            "zero_deg": "aligned_yokes_on_shared_shaft",
            "ninety_deg": "perpendicular_yokes_on_shared_shaft",
            "internal_transform": "phi_model = (90 - phi_physical) mod 180"
        },
...
def _summary_json(parameters: CardanParameters, analysis: dict[str, Any]) -> bytes:
    ...
    payload = {
        ...
        "parameters": core.parameters_to_dict(parameters),
...
def _excel_workbook(
    ...
    workbook.set_properties({
        ...
        "author": "Furkan Kasap",
        "comments": f"Generated by Cardan Joint Engineering Tool v{APP_RELEASE}",
    })
    ...
    summary_rows = [
        ...
        ("Sürüm" if language_code == "tr" else "Version", APP_RELEASE, value_fmt),
...
# One-time migration for browser sessions created before the physical-phase convention migration. Previous
# releases stored the shifted analytical phase reference in the UI. Convert any
# surviving widget values so the same physical configuration is preserved.
if st.session_state.get("_physical_phase_convention_version") != PHYSICAL_PHASE_CONVENTION_VERSION:
    for _phase_key in ("phi1_deg", "phi2_deg"):
        _slider_key = f"{_phase_key}_slider"
        _number_key = f"{_phase_key}_number"
        _stored_value = None
        if _number_key in st.session_state:
            _stored_value = float(st.session_state[_number_key])
        elif _slider_key in st.session_state:
            _stored_value = float(st.session_state[_slider_key])
        if _stored_value is not None:
            _physical_value = float(core.model_phase_to_physical_phase_deg(_stored_value))
            st.session_state[_slider_key] = _physical_value
            st.session_state[_number_key] = _physical_value
    st.session_state.analysis_data = None
    st.session_state.analysis_signature = None
    st.session_state.excel_export_bytes = None
    st.session_state.excel_export_key = None
    st.session_state._physical_phase_convention_version = PHYSICAL_PHASE_CONVENTION_VERSION
...
with st.sidebar:
    ...
    active_beta_values = [beta1_deg]
    if mode >= CardanMode.DOUBLE:
        active_beta_values.append(beta2_deg)
    if mode is CardanMode.TRIPLE:
        active_beta_values.append(beta3_deg)
    maximum_beta = max(active_beta_values)
    if maximum_beta > core.BETA_WARNING_THRESHOLD_DEG:
        st.warning(text["beta_warning"])
    elif maximum_beta > core.BETA_INFORMATION_THRESHOLD_DEG:
        st.info(text["beta_info"])
    ...
    if mode >= CardanMode.DOUBLE:
        ...
        st.caption(text["phase_reference_note"])
...
if analysis is None or analysis_signature is None:
    ...
    with tabs[3]:
        st.info(f"**{text['viewer_planar_title']}** — {text['viewer_planar_info']}")
        ...
        if viewer3d is None:
            st.warning(text["viewer_module_missing"])
        elif not viewer3d.plotly_available():
